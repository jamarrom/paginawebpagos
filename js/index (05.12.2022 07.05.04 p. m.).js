	...
	OpenPay.setId('mtpzggnxysmyhwh2w607');
    OpenPay.setApiKey('pk_7db9166e45974d3baf58d30418f00f08');
	...
	function onSuccess(response) {
		...
		$.ajax({
			data	: {	txtTitular:$("#txtNombre").val(),totalCompra:totalCompraGen,listaComprar:listaComprasText(),source_id:response.data.id,device_session_id : deviceSessionId,dataPersona:JSON.stringify(datosPerson) },
			type	: "post",
			dataType: "json",
			...
			success	: function(Respuesta)
			{
				...
				if(Respuesta[0]['texto']=="Exito"||Respuesta[0]['texto']==">Exito")
				{
					...
					localStorage.setItem('customer_id',Respuesta[0]['customer_id']);
					localStorage.setItem('order_id',Respuesta[0]['orde_id']);

					//localStorage.setItem('listaCompras', JSON.stringify(listaCompras));

					localStorage.setItem('myjson', JSON.stringify(cargarDatosFinalesCompraText(response.data.id,Respuesta[0]['customer_id'],Respuesta[0]['orde_id'],Respuesta[0]['openpay_id'])));
					localStorage.setItem('comission',comisionCompraGen);
					localStorage.setItem('total',totalCompraGen);
					localStorage.setItem('payed_by',$("#txtNombre").val());
					localStorage.setItem('payed_by_email',$("#txtEmail").val());
					localStorage.setItem('payed_by_phone',$("#txtTelefono").val());

					localStorage.setItem('datosDiv',cargarDatosCompraFinalesText());

					window.location.href = Respuesta[0]['url'];


					//swal("Carrito de compras", "Muchas gracias, su compra a sido realizada con éxito.", "success");
	
					/*cargarDatosFinalesCompra(response.data.id,Respuesta[0]['customer_id'],Respuesta[0]['orde_id']);
					borrarCookieCarrito();
			...
			error: function(XMLHttpRequest, textStatus, errorThrown) { 
			    ...
			    $(".Cargado img").slideUp('slow');
			    swal("Carrito de compras", "Lo sentimos su compra no pudo ser realizada, transcasión declinada, intente más tarde por favor..", "warning");
			}
		...
		//alert('Operación exitosa '+response.data.id);
    ...
    {		
		...
		$(".Fase-Select").eq(1).click(function(){
			...
			{
				swal("Aviso:", "Es necesario indicar el número de pasajeros.","warning");
				vt3 =  false;
			}

			if(!(($('input:radio[name=rdViaje]:checked').val()=="Si" || $('input:radio[name=rdViaje]:checked').val()=="No"))) {
				swal("Aviso:", "Es necesario indicar si el viaje es redondo","warning");
				vt2 =  false;
			}	
	...
	function cargarDatosCompra() {
		...
		for(var j=0; j<numPer; j++){
			...
			itemList += "<div class='Fase-ColListCompras u-inline-block'>";
				...
				itemList += "</p>";
				itemList += "<p style='font-family: open_sansregular;'>";
			...
			if($('input:radio[name=rdViaje]:checked').val()=="Si"){
				...
				itemList += "<div class='Fase-ColListCompras u-inline-block'>";
					...
					itemList += "</p>";
					itemList += "<p style='font-family: open_sansregular;'>";
					itemList += "El día "+diaSemana($("#txtFechaVuelta").val())+" a las "+horaDelDia(horaV)+" UNIDAD "+$('#ih-vehicle_idV').val()+" - ASIENTO "+$("#Fase-ListAsientosVueltaN .Fase-ItemAsiento.activo").eq(j).html()+"";
					itemList += "</p>";
		...
		var subTCom = ((subTotalCompra * comisionCompraT)+2.5);
		comisionCompra = (subTCom * 0.16) + subTCom;
	...
	*/
	...
	function cargarDatosCompraFinalesText() {
		var itemList = "";
...
function cargarDatosFinalesCompra(token,customer_id,order_id,id_openpay) {
	...
	/*$.post( "https://admintest.acxede.mx/api/v1/guest/venta/guardar", myJson, function(data) {
		console.log('Hurray!!!... se ejecuto el ajax')
		console.log("esto me trae el response", data);
...
function cargarDatosFinalesCompraText(token,customer_id,order_id,id_openpay) {
...
function cargarDatosFinalesCompraGuardar(myJson) {
